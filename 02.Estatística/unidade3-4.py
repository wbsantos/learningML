...
def margemErroMedia_DPConhecido(amostra, desvioPadraoPopulacao, confianca, mediaAmostral):
    alpha = 1 - confianca
    E = Z(1 - alpha) * (desvioPadraoPopulacao / (amostra ** (1/2))) 
    #print(E) # 2.5477400397891516
    return [round(mediaAmostral - E, 2), round(mediaAmostral + E, 2)]

def margemErroMedia_DPDesconhecido(amostra, desvioPadraoAmostra, confianca, mediaAmostral):
    alpha = 1 - confianca
    E = T(amostra, 1 - alpha) * (desvioPadraoAmostra / (amostra ** (1/2))) 
    #print(E) # 2.5477400397891516
    return [round(mediaAmostral - E, 2), round(mediaAmostral + E, 2)]

def margemErroProporcao(amostra, totalOcorrencias, confianca):
    p = totalOcorrencias / amostra
    alpha = 1 - confianca
    valorCritico = Z(1 - alpha)
    E = valorCritico * (((p * (1 - p)) / amostra) ** (1/2))
    return [round(p - E, 3), round(p + E, 3)]

def margemErroTamanhoAmostral():
    p = 0
    n = 0
    E = z * (((p * (1 - p)) / n) ** (1/2))


def tamanhoAmostral(confianca, margemErro):
    p = 0.5
    E = margemErro
    alpha = 1 - confianca
    n = ((Z(1 - alpha) ** 2) * p * (1 - p)) / (E ** 2)
    return math.ceil(n)

def tamanhoAmostralComVariancia(confianca, margemErro, variancia):
    desvioPadrao = variancia ** (1/2)
    E = margemErro
    alpha = 1 - confianca
    n = ((Z(1 - alpha) * desvioPadrao) / E) ** 2
    return math.ceil(n)
    
def testeHipoteseMedia():
    mediaAmostra = 7.25
    mediaTestada = 7
    desvioPadrao = 1.052
    n = 27
    localZ = round((mediaAmostra - mediaTestada) / (desvioPadrao / (n ** (1/2))), 2)
    print(localZ)
    #0.3907

def testeHipoteseProporcao():
    pAmostra = 0.1
    pTestada = 72/500.0
    n = 500
    localZ = (pAmostra - pTestada) / (((pTestada * (1 - pTestada)) / n) ** (1/2.0))
    print(localZ)
    #0.3907

print(tamanhoAmostralComVariancia(0.99, 0.05, 0.1764))
#1 27.45~32.55
#2 22.35~23.45
#3 0.056~0.144
#4 1076
